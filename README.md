...
To run the codes, one needs to add the network analysis tool box, which can be downloaded at https://aeolianine.github.io/octave-networks-toolbox/. The folder of this toolbox should be placed in the same directory as "main.m" and renamed as "NetworkAnalysisTool". Notice that this toolbox is designed in Octave, thus some commands do not work in Matlab. If the compiler reports an error regarding "printf", please comment that line in the toolbox.
